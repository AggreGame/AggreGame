$(document).ready(function() {
    ...
    $("#search-bar-wrapper").on("click", ".collection-item", function() {
    	populatePage($(this).attr("id"), true);
    });

    function populatePage(elementId, isId) {
		...
		if (isId) {
			$.ajax(databaseSettings).done(function (response) {
				writeQueryResultsToPage(response);
			});
		} else {
			databaseSettings.url = "https://igdbcom-internet-game-database-v1.p.mashape.com/games/?search=" + elementId;
			$.ajax(databaseSettings).done(function (response) {
    ...
    function writeQueryResultsToPage(response) {
	 	console.log(response);
	 	var url = "https://images.igdb.com/igdb/image/upload/t_cover_big/" + response[0].cover.cloudinary_id;
